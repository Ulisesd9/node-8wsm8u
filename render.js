...
function recordData() {
  ...
  client.on('message', function (topic, message) {
    ...
    if (end == false) {
      ...
      if (indexOfMax != 0 && confidence[indexOfMax] >= 0.8) {
        if (consecutiveCounter == 0) {
          prevLabel = label[indexOfMax];
        }
        currentLabel = label[indexOfMax];

        if (currentLabel == prevLabel) {
          if ((consecutiveCounter = 0)) {
            DectectStartTime = Date.now() - startTime;
          }
          consecutiveCounter++;
          ...
          detectTime[counter] =
            DectectStartTime.toString + ':' + DectectEndTime;
  ...
  switch (label) {
    case 'cracks-breaks-collapses':
      ...
      document.getElementById('label').innerHTML =
        'Crack-Breaks-Collapses  ' + String(confidence) + ' %';
      break;
    case 'root':
      var x = document.getElementById('camera');
      x.style.border = '2px solid #0984e3';
      document.getElementById('label').innerHTML =
        'Root  ' + String(confidence) + ' %';
      break;
    case 'obstacle':
      var x = document.getElementById('camera');
      x.style.border = '2px solid #00b894';
      document.getElementById('label').innerHTML =
        'Obstacle  ' + String(confidence) + ' %';
      break;
    case 'displace':
      var x = document.getElementById('camera');
      x.style.border = '2px solid #8c7ae6';
      document.getElementById('label').innerHTML =
        'Displace  ' + String(confidence) + ' %';
      break;
    case 'normal':
      var x = document.getElementById('camera');
      x.style.border = '2px solid #2d3436';
      document.getElementById('label').innerHTML =
        'Normal  ' + String(confidence) + ' %';
      break;
    default:
      var x = document.getElementById('camera');
